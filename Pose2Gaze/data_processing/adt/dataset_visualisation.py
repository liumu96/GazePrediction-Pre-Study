...
class Player_Skeleton:
    ...
    def play_xyz(self, path, pose_xyz, gaze, head, draw_every=1, ray_len=0.5, dot_radius=20):
        
        # 从 head 位置出发的两条方向线（长度可调）
        gaze_direction = pose_xyz[:, 4*3:5*3] + gaze[:, :3] * ray_len
        head_direction = pose_xyz[:, 4*3:5*3] + head[:, :3] * ray_len
        pose_xyz = np.concatenate((pose_xyz, gaze_direction), axis = 1)        
        pose_xyz = np.concatenate((pose_xyz, head_direction), axis = 1)
        
        useful_frames = gaze[:, 5].astype(int)  # 避免浮点索引
        ...
        for i in range(pose_xyz.shape[0]):        
            ...
            # 每 draw_every 帧刷新一次右侧 ego 图像
            if self.show_images and i % draw_every == 0:
                ...
                try:
                    ...
                    # 画 2D gaze 点
                    gaze_x = float(gaze[i, 3]) * img_width
                    gaze_y = float(gaze[i, 4]) * img_height
                    circle = patches.Circle([gaze_x, gaze_y], radius=dot_radius, facecolor="red", alpha=0.7)
                    self._ax1.add_patch(circle)
                except Exception as e:
                    # 没有保存该帧图像也不要崩
                    self._ax1.text(0.5, 0.5, f"Image not found:\n{frame}.png",
                                   ha='center', va='center', transform=self._ax1.transAxes)
                    self._ax1.axis('off')
...
if __name__ == "__main__":
    data_path = "../../../../Datasets/scratch/pose_forecast/adt_pose2gaze/train/Apartment_release_work_skeleton_seq106_M1292_"

    gaze_path = data_path + 'gaze.npy'
    head_path = data_path + 'head.npy'
    pose_path = data_path + 'pose_xyz.npy'
    show_images = True # display the ego-centric images
    if show_images:
    ...
    player.play_xyz(data_path, pose, gaze, head, draw_every=1, ray_len=0.5, dot_radius=14)